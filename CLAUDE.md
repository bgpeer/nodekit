...
仓库主人原话：「写这么多干什么，后面这个提示用高亮」；「写得乱糟糟的，都给我改成精简易懂版」。

- 菜单：标题一行；每项一行「编号. 名称　当前：值」；**选项下面不写解释行**。
- 要提醒的只留**一行**，用 `tip()`（黄色粗体「提示：…」）。
- 危险操作：一行 `tip()` 说清后果 + `ask_yn(…, False)`，确认照样要，不许为了精简把确认删掉。
- 操作结果一行 ✔ / ⚠。原委、实测数据、踩坑经过写进代码注释，不上屏。
- 精简的是字，不是功能：选项、入口、确认一个都不许少（见第四节）。
