...
#
# 探测被拒 = 条目里【一条音视频轨都没有】。而这【不等于播不了】—— 曾经把它写成
# "必定 load fail"，实测被推翻了：一个 0 轨道的条目照样连续播了 40 秒。
# 真相是 Emby 在你点播放那一刻会【现场探一次】：源那会儿给得出数据就能播（开播多等
# 几秒），正在限流或挡 UA 就是 load fail。也就是每次播放都押一次运气 ——
# 而上游按 UA 挡人的那几天，这个运气一次都押不中，于是看着像"必定播不了"。
print()
