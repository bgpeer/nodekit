...
def do_heal_trace(q, play=True):
    ...
    if _ht:
        # 【是"动手"不是"跑"】它每分钟都跑，没人点开就一眼看完退出、不留记号
        _trace_row("·", "上一次动手", f"{int(time.time() - _ht)} 秒前"
                   f"{DIM}（每分钟都在看；没人点开就不动手、不记这个）{RST}")
    else:
        _trace_row("⚠", "上一次动手", "一次都没动过手（或者是升级前的版本，没记这个）")
    ...
    if "log" not in ev:
        ...
        if any(s == "heal-tick" for _p, s, _x in running_tasks()):
            ...
            print(f"  {DIM}读走了，但没有去探它。上面「流水」那几行里要是有"
                  f"「没探：……」，那就是原因；上面要是有「冷却中」，就是在冷却。{RST}")
            print(f"  {DIM}等不及就点名补：media-stack heal {q}（不受额度限制）{RST}")
    else:
...
# 【只补你点开过的】补时长的唯一用途是进度条和续播记忆 —— 而没点开过的条目，
# 补出来的进度条给谁用？库里两千多部，点开过的几十部：这一条把每天的流量从 GB 压到 MB。
#
# 【它不会让人少看一部片】没探到的条目照样能播：Emby 在你点播放那一刻会现场探一次
# （见 items_without_duration 的说明）。代价只是开播前多等几秒。而那一次点开【本身】
# 就把它放进了播放记录，下一轮就补上 —— 需要进度条的那一刻，进度条就在。
#
# 【曾经写成"继续观看 + 收藏 + 最近 7 天加进来的"】那是个漏洞：刚建完库的那一刻全库
# 都是"最近加进来的"，于是那一档等于全库都补，省流量的承诺当场落空。而"收藏"也不是
# "看过"。判据只有一个：你点开过它没有。


def _played_ids(key):
    """所有账号里【点开过】的条目 id：继续观看 + 已看完。

    【必须遍历每个账号】一台机器上电视一个号、手机一个号是常态，只认第一个号会漏掉
    一半 —— 那半边的片子永远等不到进度条。

    【只问 Emby，不碰网盘】读的是 Emby 自己的库，一个字节都不出境，所以每轮无脑跑一次。
    """
    out = set()
    try:
    ...
    for u in users:
        ...
        if not uid:
            continue
        for path in (f"/Users/{uid}/Items/Resume?Limit=500"
                     f"&IncludeItemTypes=Movie,Episode,Video",
                     f"/Users/{uid}/Items?Recursive=true&Filters=IsPlayed&Limit=2000"
                     f"&IncludeItemTypes=Movie,Episode,Video"):
            try:
...
def items_without_duration(key, scope=None):
    ...
    （见 _played_ids）；"all" 是整个库，只给体检和账本报总数用。
    ...
    MediaStreams 是【0 条】—— 一条音视频轨都没有。

    【轨道为 0 不等于播不了，等于每次点开都要现场探一次】这一条曾经写成"根本播不了"，
    是错的：实测一个 0 轨道的条目（1.73 GB / 119 分 / 2.0 Mbps）连续播了 40 秒，同一
    时刻 nginx 日志里有播放器的 206。真相是 Emby 在客户端问 PlaybackInfo 那一刻会【实时
    探测】，所以：

      · 那一刻源给得出数据 → 现场探到 → 能播，只是开播前多等几秒
      · 那一刻源在限流 / 挡 UA / 超时 → 探不到 → load fail

    也就是说 0 轨道的条目【每次播放都在押一次运气】，而且每次都要额外向网盘拉一段。
    这解释了之前所有的现象：上游按 UA 挡人那几天，每次现场探测都失败、于是每次都
    load fail；UA 改写 + 退避之后源肯给数据了，同一个 0 轨道的条目就播出来了。

    所以这一步仍然值得做 —— 补上之后 Emby 直接拿库里那份，开播更快、也不用每次押运气。
    ...
    if not uid:
        return out
    # 【筛选只问 Emby，不碰网盘】播放记录读的是 Emby 自己的库，一个字节都不出境。
    want = None if scope == "all" else _played_ids(key)
    for lb in libs:
        pid = lb.get("ItemId")
...
def heal_how(items):
    """这一轮是谁触发的：看片后 / 点名 / 整队。流水、额度、体检都按它分。

    【看片后 ≠ heal-tick 跑的每一轮】tick 在没有刚点开的缺信息条目时，会往下跑整队
    那一截。以前那一截也记成「看片后」，而刚点开的那几条反倒因为传了 items 记成了
    「点名」—— 两个标签正好贴反。标签反了，额度就分不对：积压会吃掉留给你刚点开
    那一集的那一截。
    """
    if items is None:
        return "整队"
...
def opt_tag(key, val):
    ...
    if is_hls_val(key, val):
        ...
        # 写死"已修好"：分片重定向是按需装的服务，没装 / 装失败 / 版本旧的机器上
        #   转码流照样播不了，那是另一个方向的假话，而且更坑（人会去查别的地方）。
        # 所以按服务在不在说：在就什么都不挂（这一档现在是好的），不在才提醒。
        return "" if hls_ready() else f"  {YELLOW}⚠ 要装分片重定向{RST}"
