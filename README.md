...
2. **三网回程路由** —— 对电信/联通/移动的测试点做 traceroute，**逐跳查真实 ASN**
   ...
   联通 169 / 移动 CMIN2 / 移动 CMI。快速模式各测北京一个点，`2 完整` 测京沪穗共 9 个点。
3. **IP 黑名单** —— 10 个 DNSBL。
...
> **测试点不盲信**：跑之前先查每个目标的真实 ASN 和注册地，对不上就标「目标存疑」并跳过。
> （公认的「北京电信」测试 IP `219.141.136.12` 实际归属是 AS4847 CNIX，不是 AS4134。）
