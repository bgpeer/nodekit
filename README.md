...
> 本项开关把它换成**本机自建中转**——用你自己的域名 `https://本机域名:订阅端口/gh/...`，不再依赖别人的服务。
> **默认开**（有域名+真证书时）；不想用点一下即改回 `gh-proxy`（循环开关，自动刷新订阅）。中转**与订阅同端口**
> （客户端本就从这端口拉订阅，无需额外放行）、**只白名单 GitHub**（`raw.githubusercontent.com` 等，绝不做成开放代理）。
> 用本机域名动态注入、不写死公共模板，别人的部署用别人的域名。
