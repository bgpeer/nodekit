...
- **内核自动更新**：安装后自动挂 cron，**每月北京时间 2 号凌晨 04:00** 把 sing-box / xray 更新到最新并重启一次
  （无新版则跳过）；也可随时进管理面板 **13 更新核心** 手动立即更新。日志在 `/var/log/bgpeer-coreupdate.log`
...
面板标题栏会显示当前脚本版本（如 `bgpeer 一键脚本 v1.0.0`）；点 **12 更新脚本** 时会显示
`v旧 → v新`，一眼看出更到了什么版本。
...
给"哪天 VPS 的 IP 被墙"备一条后路：菜单 8 每条起一个独立的 `xy-cdn-<n>` 实例、跑一个
...
- **设备怎么用**：
  ...
  - **安卓手机「专用 DNS」（全系统加密，推荐）**：网页后台 → 设置 → 加密设置，启用、服务器名填**你的域名**、
    证书指到脚本已有的 acme 证书（`/etc/ssl/sb/acme.crt` + `.key`）、DoT 端口 `853`、DoH 端口留 `0`（不占 443）；
...
- 主菜单 `15 卸载`（整体卸载）会**一并撤掉 AdGuard Home**（它的 DoT 依赖 acme 证书，证书随整体卸载删除）。
...
```
  ...
  2 放行白名单（作者名单 / 自定义名单）
  3 自定义放行名单脚本链接
  4 立即更新（拉取最新放行名单/规则集并生效，不用等每天定时刷新）
  5 卸载（不想屏蔽了，直接清掉规则）
  0 退出
```
...
管理面板选 **14. 卸载**，会移除本脚本安装的 sing-box / xray / 订阅服务、配置、证书、
