...
- **可加多条**：`1 CDN节点安装` 里**协议可多选**（逗号分隔如 `1,3,4`、或 `a` 全选），一次把选中的都装上
  ...
  `2 查看` 上方列已配置节点（含各自的优选地址）、下方列全部备用链接；`4 卸载` 可选删某条、**多条（逗号分隔如 1,3）**或全部；`5 优选地址`（手动填 / 测速筛候选）见下。
...
- **`1 手动填`**：填第三方公开的优选域名，或某个具体 CF IP，写进**全部 CDN 节点**。
  写入前先从本机 TCP 握手探一下，不通会警告让你确认（本机不通 ≠ 客户端不通，最终由你拍板）。
  **留空回车 = 取消优选**、恢复用域名。已经在客户端侧跑过优选工具、心里有数的，用这个最直接。

- **`2 测速筛候选`**（推荐）：本机粗筛出若干个还不错的 CF 边缘，**每个生成一条节点写进订阅**
  （`CDN·优选1` … `优选N`），**最终由客户端的 URLTest 挑最快的那条**。
...
> 每次执行前都会先告诉你本轮上限并让你确认。流量包紧的把名次和下载量调小，或者干脆只用 `1 手动填`。
