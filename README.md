...
#### 想在模板里自己写国家组？同名即接管（mihomo）

脚本生成前会先扫一遍模板 `proxy-groups:` 段里已有的组名，**同名的就不再自动生成**，让你写的那个说了算：

- **同名**（模板里写 `🇺🇸美国随机`）→ 脚本**不再生成**同名组，样式完全由你定（类型、`hidden`、`filter` 随便改）。
  它仍会被拼进 `🌍全球加速` 的列表，位置和作用跟原来一样。
- **不同名**（模板里写 `🇺🇸美国自动`）→ 两个**并存**：你的 `🇺🇸美国自动` 和脚本的 `🇺🇸美国随机` 各管各的。
  但注意 `__XY_NAMES__` 只会拼**脚本生成的**那些组名，你自己那个要出现在别的策略组里得**手动加进对应的 `proxies:`**。

为什么要这样：mihomo 遇到**重名策略组是硬失败**（`ProxyGroup xxx: duplicate group name`），
整份配置加载不了、客户端直接起不来——不是「以某一个为准」的软覆盖。
