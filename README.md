...
- **可加多条**：`1 CDN节点安装` 里**协议可多选**（逗号分隔如 `1,3,4`、或 `a` 全选），一次把选中的都装上
  ...
  `2 查看` 上方列已配置节点（含各自的优选地址）、下方列全部备用链接；`4 卸载` 可选删某条、**多条（逗号分隔如 1,3）**或全部；`5 优选地址` 见下。
...
**服务端配置、证书、CF 后台一律不用动**，三条订阅和单条链接同步生效。

- **`1 手动填`**：填第三方公开的优选域名，或某个具体 CF IP。写入前先从本机 TCP 握手探一下，
  不通会警告让你确认（本机不通 ≠ 客户端不通，最终由你拍板）。**留空回车 = 取消优选**、恢复用域名。
- **`2 立即自动测速优选`**：拉 **Cloudflare 官方 IP 段**（`cloudflare.com/ips-v4`，拉不到用内置兜底段）
  → 切成 `/24` 打散随机采样（默认 500 个，覆盖面比按段均分广）→ **多线程 TCP 握手筛延迟**
  → 延迟最优的前几名跑 **`speed.cloudflare.com` 下载测速** → 挑吞吐最高的写入。
  测速端口默认沿用你 CDN 节点的 CF 端口（顺带验证该边缘对这个端口确实放行），全挂时自动用 443 复测一轮。
- **`3 定时自动优选`**：cron 每 3/6/12/24 小时自动跑一轮并刷订阅，客户端下次拉订阅就换上新地址。
  分钟数随机错峰。删光最后一条 CDN 节点时会自动关掉，不会留个空跑的定时任务。
- **`4 测速参数`**：候选数 / 名次 / 并发 / 超时 / 单个下载量 / 时长 / 采纳下限 / 测试端口，都能调，范围有校验。

> ⚠️ **下载测速是真的在下数据、走你 VPS 的流量**：一轮最多 `名次 × 单个下载量`，默认 6×10 = **60MB**，
> 菜单里每次都会先告诉你本轮上限并让你确认。流量包紧的把名次和下载量调小，或者干脆只用手动填。

> ⚠️ **测的是"你的 VPS → CF 边缘"这一段**，不是"你家宽带 → CF 边缘"。两者常常一致（同一批边缘、
> 同一个大区），但不保证。所以：**自动优选给的是一个比默认解析更靠谱的起点，不是客户端侧最优解**；
> 真要抠到极致，还是在客户端所在网络上跑优选工具、把结果用 `1 手动填` 写进去。
> 另外测速有**下限保护**：本轮没测出可用地址、或最优速度低于你设的 `采纳下限`，就**保持现状不动**，
> 不会因为一次网络抖动把好好的地址换成个烂的。
