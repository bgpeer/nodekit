...
- **可加多条**：`1 新增` 里**协议可多选**（逗号分隔如 `1,3,4`、或 `all` 全选），一次把选中的都装上
  （共用你输入的那个域名、各占一个端口/服务）；也可反复 `1 新增` 用不同域名再加。`2 查看` 只列
  全部备用链接（干净）；`4 卸载` 可选删某条、**多条（逗号分隔如 1,3）**或全部。CF 免费版可代理端口有限，最多 5 条。
