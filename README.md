...
> 下次「更新配置」也能正常归组，不用重装。
> **老节点怎么补上分隔点**：名字是烤在分享链接里的，`更新配置` 只重新套模板、**不会重新生成名字**，
> 所以老装的节点仍是 `🇺🇸2anytls` 这种。进 **`2 节点链接 / 订阅`**，检测到就会问你要不要一键改名——
> **只改显示名，uuid / 端口 / 路径 / 服务 / 证书一律不动，不用重装**（重装会重新生成全部 uuid 和端口）。
> 改完客户端重拉订阅即可；多机聚合的还要回主机点一次 `更新配置` 重新汇总。
> 代价：客户端里手动选中过的节点会因为改名回到分组默认；自定义模板里写死了旧名字的要同步改。
