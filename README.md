...
- **内核自动更新**：安装后自动挂 cron，**每月北京时间 2 号凌晨 04:00** 把 sing-box / xray 更新到最新并重启一次
  （无新版则跳过）；也可随时进管理面板 **12 更新核心** 手动立即更新。日志在 `/var/log/bgpeer-coreupdate.log`
...
```
  ...
  13. 卸载
  14. CDN 灾备节点（域名+Cloudflare 中转，防 IP 被墙 · 备用）
  0. 退出
```
...
给"哪天 VPS 的 IP 被墙"备一条后路：菜单 14 用一个独立的 `xy-cdn` sing-box 实例起一个
**VLESS + WebSocket + TLS** 节点，套 Cloudflare 橙云中转——客户端连的是 **CF 的 IP**、不是你
VPS 的真 IP，所以真 IP 被封了这条仍能用。与主节点完全隔离（独立服务/配置/证书），重装主节点
不受影响，也能单独卸载。

- **不需要 Let's Encrypt**：CF 的 SSL 模式用 **Full（完全）**，源站自签证书即可（CF 不校验源站
  证书，客户端看到的是 CF 的有效证书）——省了 80 端口验证、证书续期一堆麻烦。
- **不需要 Origin Rule**：默认走 CF 可代理的 `2053` 端口（两端同口），避开主节点的 443/8443。
- 生成时可选**协议**（`1 VLESS+WS`·最稳 / `2 VLESS+XHTTP`·最快 / `3 VMess+WS` / `4 Trojan+WS`）、
...
- **写入订阅按客户端支持度自动筛选**：VLESS+WS / VMess+WS / Trojan+WS 三格式全支持；VLESS+XHTTP
  ...
  （客户端自己怎么用不受订阅筛选影响）。
- 生成后面板会打印 **Cloudflare 侧照做清单**（加域名、A 记录指向本机 IP、开橙云、SSL 选 Full、
  安全组放行端口）和一条**独立备用链接**——平时留着不导入，被墙那天再粘进客户端即可。
- 子菜单 `3 节点写入订阅配置`（循环开关）：想让它平时也能用，一键把这条 CDN 节点**并进三条订阅**
  并自动刷新（客户端拉一次即见）；再按一次即移出。装/卸载都会自动同步订阅，不留死节点。
...
管理面板选 **13. 卸载**，会移除本脚本安装的 sing-box / xray / 订阅服务、配置、证书、
