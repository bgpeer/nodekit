...
- **更新核心不怕 SSH 断**：更新的最后一步是重启核心，而很多人是**挂着本机代理管理这台机**的，重启会当场掐断 SSH。
  菜单 17 会把更新**派到独立会话里执行**（脱离控制终端，SIGHUP 打不到它），断开后仍在服务端跑完；前台只是实时跟随
...
>
> **刚往网盘里加了片子，Emby 里没有？** 先想到目录缓存：OpenList 默认把目录列表
> 缓存 12 小时（脚本自动调的，为的是少打网盘接口、避免被限流），缓存没过期之前
