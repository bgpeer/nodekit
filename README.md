...
```
  ...
  2. 节点链接 / 订阅
  3. 多路复用开关 smux（只针对 ws / httpupgrade 协议）
  4. mihomo 配置
  5. sing-box 配置
  6. 小火箭配置
...
> **管理时建议关掉本机代理**（或在代理里把这台 VPS 的 IP 设为直连/绕过）。
> 安装、smux 开关、BT 屏蔽、屏蔽中国域名、更新核心等操作都会**重启 sing-box/xray**；
> 如果你正挂着这台机器的代理来 SSH 管理，重启会掐断隧道、SSH 瞬断。脚本已把这些操作
> 改成「先把配置/状态落盘、再后台异步重启」，所以**即便 SSH 断了，操作也已在服务端完成**，
> 重连回来即是最新状态；但每次都被断很烦，直连管理最省事。
