...
**统一锚点（三格式一致，便于维护）**：
- `__XY_NODES__` — 注入节点（三格式同名，各按自己语法渲染）;
- `__XY_GROUPS__` — **自动创建国家随机分组**：扫描模板里的节点名，按国家(旗子/关键词)建 url-test 随机组，
  某国 ≥2 个才建，剩下没归类的进「🎲其他随机」;
- `__XY_GROUP_NAMES__` — 把建好的国家组名接进主选择组（mihomo「🌍全球加速」/ sing-box「🌍代理」/ 小火箭服务组）。

自定义模板只要放这几个锚点，就能白嫖国家随机分组；不放也能正常出配置。
